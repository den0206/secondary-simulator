...
- **⚙️ 端末設定**: iOS/Android の外観・文字サイズ、iOS 26 以降の Liquid Glass、主要都市プリセット付きの模擬位置情報、iOS シミュレータのソフトウェアキーボード表示を設定
- **🎬 記録**: スクリーンショットと画面録画を、指定した保存先へ直接書き出し
- **💾 メモリ効率**: ストリーム・リスナー・タイマーのクリーンアップでメモリリークを防止

## 📋 要件

- **VSCode / Cursor**: VSCode バージョン 1.90.0 以上
- **Node.js**: 開発・CI は 24 LTS。拡張の実行互換性は `engines.node >=20` を維持
- **macOS**: iOS Simulator の利用と HID 直接注入に必要（Android のみなら他 OS でも動作しますが未検証）
- **mobilecli**: macOS・Linux・Windows の x64 / arm64 バイナリを VSIX に同梱。
  通常の起動には別途 Node.js/npm のインストールやパッケージのダウンロードは不要。
  同梱バイナリが欠落・実行不可の場合は固定版の `npx` フォールバックを試みる
  （npm とネット接続が必要。警告としてログに残る）。
...
4. ドロップダウンメニューからデバイスを選択（ビューのタイトルバーの更新アイコンで一覧を再取得）。Auto ON なら起動中のデバイスへ自動で繋ぐ
...
6. マウス/タッチでデバイスと対話：
   ...
   - **長押し**: 押下したまま静止でロングプレス
   - **Home**: プレビュー下のボタン。Xcode 27 の iOS Simulator では同梱 mobilecli agent を使う。
     agent が無い場合は出力チャンネルに導入コマンドが表示される
...
VSIX には `mobilecli` 1.0.13 を同梱しており、こちらは **FSL-1.1-ALv2** です。
