...
(async () => {
  ...
  // 接続1: <img src=...> 相当。**これが mobilecli の起動待ちも兼ねる。**
  // 別に「ウォームアップ用の接続」を張って捨てる形にはしない。セッションを 1 つ
  // 余計に開け閉めするだけで、待ちが要る理由（初回接続で端末側の WDA が起動する）
  // は変わらないため。
  let f1 = 0;
  ...
  r1.on('error', () => {});
  // **枚数の下限を 1 にしてあるのは、見たいのが「流れる／止まる／また流れる」だから。**
  // GitHub の macOS ランナーでは WDA 経由の MJPEG が細切れにしか出ない
  // （実測: ローカルは 6 秒で 162 枚＝約 27fps、CI は 60 秒で 2 枚）。
  // ここで fps を要求すると、切断と復帰ではなくランナーの遅さを測ることになる。
  await waitFor(() => f1 > 1, 180000);
  const framesBefore = f1;
  check('接続1がフレームを受信', framesBefore > 1, `frames=${framesBefore}`);
  ...
  r2.on('error', () => {});
  // 復帰したかどうかが論点なので、こちらも枚数は問わない（上のコメント参照）
  await waitFor(() => f2 > 1, 180000);
