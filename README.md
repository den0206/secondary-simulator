...
- **⚙️ Device settings**: set appearance and text size on iOS/Android, Liquid Glass opacity on iOS 26+, simulated location with major-city presets, and the on-screen software keyboard on iOS simulators
...
- **mobilecli**: macOS, Linux, and Windows binaries (x64 / arm64) ship inside the VSIX;
  normal startup does not require a separate Node.js/npm installation or a package download.
  If the bundled binary is missing or not executable, a pinned `npx` fallback is attempted
  (requires npm and network access; logged as a warning).
...
- **`secondarySimulator.recordingSource`**: where a recording comes from — `view` (default) records what the sidebar shows, including the mouse cursor and tap markers (and the drag trail when `secondarySimulator.showTouchTrail` is on); `device` records on the device, at full device resolution, but the cursor and taps are not part of the device's own screen, so they never appear. The `view` route encodes in the webview, so the file is MP4 or WebM depending on what this Chromium can encode. While a recording runs, the sidecar capture keeps at least 1080 pixels of width even when the panel is narrow, so a narrow sidebar no longer decides the resolution of the file (the WebDriverAgent and Android paths have no equivalent knob, so there the picture still follows the stream you see). Pick `device` when you want the device's full resolution and do not need the cursor.
...
| `Record Screen`       | Start/stop recording the screen to a video file. Starts after a three-second countdown; stops automatically after 10 minutes or on disconnect. The cursor and taps are included unless `secondarySimulator.recordingSource` is `device` |
...
The VSIX bundles `mobilecli` 1.0.13, which is licensed under **FSL-1.1-ALv2**.
