...
> **2026-08-17 追記: 1〜8 を実装した。** 5 の `captureConfig` と、その前提になる
...
4. **[未対応] サイドカープロセスを使い回す**（§2.9）。非表示・デバイス切替では
...
4 は未対応のまま残した。プロセスの寿命をデバイスから切り離す変更で、
HID 降格の流れ（`onBackendChange` → 映像だけ WDA へ付け替え）と絡むため、
他と混ぜると壊したときの切り分けが難しくなる。単独で入れる。
