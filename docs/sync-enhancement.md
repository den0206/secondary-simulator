...
> **2026-08-17 追記: 9・10 を実装した。11 は既定を切り替えてから。**

9. **[実装済]** サイドカーに loopback HTTP（トークン必須）を持たせ、
   `multipart/x-mixed-replace` を配信する（`captureServe` / `sink: "http"`）。
   ポートとトークンは拡張ホストが決めて渡し、埋まっていれば次を試す。
   張れなければ `stdout` 経路で開始し直す。
10. **[実装済]** `directStream` の判定から「サイドカーのときは無効」を外した（§2.1）。
    取り込み元（サイドカー / WDA）と転送（直結 / 個別配信）を独立に選べる。
11. **[未対応]** stdout を制御専用に戻し、`emitFrameLine` と trylock を削除する（§2.4）。
    **実機で直結を確認し、`directStream` を既定にしてから**。今は既定が `false` なので
    stdout 経路が主で、消すと逃げ場が無くなる。

**見込み**: 直結時はコピー 4 回 → 1 回。入力応答が映像の背圧から独立する。
ただし**既定は従来どおり**（`directStream` は実験扱いのまま）なので、
既定のままでは §2.4 は残る。

### Phase C — プッシュ型取り込み（§3.1）／必要なら H.264（§3.3）

> **2026-08-17 追記: 12 を実装した。13 は着手していない（理由は下記）。**

12. **[実装済]** `registerCallbackWithUUID:damageRectanglesCallback:` /
    `…ioSurfaceChangeCallback:` によるプッシュ化（`mode: "auto"`、既定）。
    存在確認・引数を触らないコールバック・自己点検からのポーリング復帰の 3 段構え。
    `secondarySimulator.captureMode` で `poll` に固定できる。
13. **[未着手]** H.264 + WebCodecs。**Phase A-1 の計測で足りないと分かってから**、が
    このレポート自身の判断（§3.3）。現時点で実機の計測値が無く、
    帯域は既に 0.5MB/s まで落ちている。ハードウェアエンコーダと WebCodecs を
    実機確認なしで足すのは、得るものより壊すものが大きい。

**見込み**: 取り込み遅延 -16.7ms（平均）、静止時は毎秒 2 枚の自己点検ぶんだけ。

#### プッシュ型の自己点検について（実装上の注意）

当初は「`IOSurfaceGetSeed` が変わったのに通知が来ない」で検出する設計にしたが、
**これは誤り**。seed は中身が同じでも動く（§2.2 でそう書いた当人の設計）ため、
静止画面で誤検知してポーリングへ落ちてしまう。実装では
**「しばらくフレームが出ていないときだけ 1 枚撮ってみて、出たかどうか」**で見る。
通知が効いていれば絵が動いている間はフレームが流れるので、そもそもここへ来ない。
...
0. **ネイティブ変更がコンパイルできて動くか。** 本環境では `npm run build` の native が
   スキップされるため **Phase A〜C のネイティブは未コンパイル**。順に、
   (a) `captureConfig` / `captureAlive` / descriptor 取り直し（Phase A）、
   (b) `captureServe` の直結配信（`secondarySimulator.directStream` を on にして
   `<img>` に絵が出るか。フッターが「直結」表示になるか）、
   (c) プッシュ型（`[simhid] 変更通知で取り込む` が stderr に出るか。
   出ない場合は `respondsToSelector:` が false なのでポーリングのまま＝安全側）。
   (c) が動いた場合は「ポーリングへ戻す」ログが出ないことも確認する。
